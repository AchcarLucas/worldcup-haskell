...
selectUsersByDistance limit page lgt lng = do 
	runDB $ rawSql "SELECT ?? FROM public.user WHERE (SELECT COUNT(*) FROM public.figure_user WHERE user_id = public.user.id) > 0 ORDER BY (12742 * asin(sqrt(0.5 - cos ((gps_latitude - ?) * 0.017453292519943295) / 2 + cos (? * 0.017453292519943295) * cos(gps_latitude * 0.017453292519943295) * (1 - cos((gps_longitude - ?) * 0.017453292519943295)) / 2))) ASC LIMIT ? OFFSET ?" [toPersistValue lgt, toPersistValue lgt, toPersistValue lng, toPersistValue limit, toPersistValue $ limit*page]

selectUsersByFigure :: Int -> Double -> Double -> Handler [Entity User]
selectUsersByFigure fid lgt lng = do 
	runDB $ rawSql "SELECT ?? FROM public.user WHERE (SELECT COUNT(*) FROM public.figure_user WHERE user_id = public.user.id AND figure_id = ?) > 0 ORDER BY (12742 * asin(sqrt(0.5 - cos ((gps_latitude - ?) * 0.017453292519943295) / 2 + cos (? * 0.017453292519943295) * cos(gps_latitude * 0.017453292519943295) * (1 - cos((gps_longitude - ?) * 0.017453292519943295)) / 2))) ASC" [toPersistValue fid, toPersistValue lgt, toPersistValue lgt, toPersistValue lng]
